...
#
# History:
#               - 0.4: Fixed whois command, wasn't working properly
...
namespace eval ctcommands {
	...
	proc pub:ban {nick host hand chan text} {
		...
		set var0 [lindex [split $text] 0 ]
		if {![botisop $chan]} {
		} elseif {[isbotnick $var0] && [matchattr [nick2hand $var0] $ctcmd(flag)]} {
			return }
		if {[onchan $var0 $chan]} {
			set bhost "[maskhost $var0![getchanhost $var0 $chan] $ctcmd(btype)]"
			if {[string match -nocase *!*@* $bhost]} {
				return }
